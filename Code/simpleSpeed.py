...
class CAStochastic(object):
    ...
    def localIRule(self,k):
        ...
        ones = np.ones(int(self.nClustOnes[k-1]))
        minus = np.ones(int(self.clusterSize[k-1] - self.nClustOnes[k-1]))

        c += np.sum(np.dot(a,ones)) #vector product ones
        c += np.sum(np.dot(-a,minus)) #vector product minus ones
